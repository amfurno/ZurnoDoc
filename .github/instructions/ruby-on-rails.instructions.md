...
- Write unit tests for models using `spec/models` (RSpec) to validate business logic.
- Use factories with `FactoryBot` (RSpec) to manage test data cleanly and consistently.
- Organize controller specs under `spec/requests` to test RESTful API behavior.
- Prefer `before` blocks in RSpec to initialize common test data.
- Avoid hitting external APIs in tests — use `WebMock`, `VCR`, or `stub_request` to isolate test environments.
- Use `feature specs` with Capybara in RSpec to simulate full user flows.
...
- Avoid `sleep` in tests; use `ActiveJob::TestHelper` with RSpec.
...
- Use custom matchers (RSpec) for reusable and expressive test logic.
